...
public sealed class MailboxServiceTestsPBI019
{
    ...
    {
        _repo.Setup(r => r.GetPagedAsync(It.IsAny<int>(), It.IsAny<int>(), null, null, It.IsAny<PostRoute.DAL.Entities.MailboxStatus?>(), null, false, It.IsAny<CancellationToken>()))
             .ReturnsAsync((new List<Mailbox>(), 0));

        var result = await _sut.GetPagedAsync(0, 25, null, null, null, null, false, CancellationToken.None);

        Assert.Equal(1, result.Page);
        _repo.Verify(r => r.GetPagedAsync(1, 25, null, null, It.IsAny<PostRoute.DAL.Entities.MailboxStatus?>(), null, false, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetPagedAsync_ClampsPageSizeBelow1To25()
    {
        ...
        var result = await _sut.GetPagedAsync(1, 0, null, null, null, null, false, CancellationToken.None);
    ...
    public async Task GetPagedAsync_ClampsPageSizeAbove100To100()
    {
        ...
        var result = await _sut.GetPagedAsync(1, 500, null, null, null, null, false, CancellationToken.None);
    ...
    public async Task GetPagedAsync_PassesAllFiltersToRepository()
    {
        _repo.Setup(r => r.GetPagedAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<MailboxType?>(),
                                          It.IsAny<MailboxPriority?>(), It.IsAny<MailboxStatus?>(), It.IsAny<string?>(), It.IsAny<bool>(),
                                          It.IsAny<CancellationToken>()))
             .ReturnsAsync((new List<Mailbox>(), 0));

        await _sut.GetPagedAsync(2, 10, MailboxType.SpecialPriority, MailboxPriority.Visok, MailboxStatus.Pun, "Sarajevo", true, CancellationToken.None);

        _repo.Verify(r => r.GetPagedAsync(2, 10, MailboxType.SpecialPriority, MailboxPriority.Visok, MailboxStatus.Pun, "Sarajevo", true, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetPagedAsync_ReturnsItemsAndTotalsFromRepository()
    {
        ...
        _repo.Setup(r => r.GetPagedAsync(1, 25, null, null, It.IsAny<PostRoute.DAL.Entities.MailboxStatus?>(), null, false, It.IsAny<CancellationToken>()))
             .ReturnsAsync((items, 42));

        var result = await _sut.GetPagedAsync(1, 25, null, null, null, null, false, CancellationToken.None);
