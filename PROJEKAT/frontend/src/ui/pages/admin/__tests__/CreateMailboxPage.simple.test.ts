 ...
 * Ovo su jednostavni testovi koji ne zahtevaju dodatne biblioteke
 * Mogu se pokrenuti sa: ts-node --project tsconfig.json src/ui/pages/admin/__tests__/CreateMailboxPage.simple.test.ts
 */
...
// Mock funkcije
const mockCreateMailbox = async (data: typeof mockMailboxData) => {
...
const mockCheckSerialNumber = async (serialNumber: string) => {
...
async function testNotesHandling() {
  ...
  // Test with empty string notes (instead of null)
